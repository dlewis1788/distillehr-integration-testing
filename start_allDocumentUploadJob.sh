...
_USER=admin
_PASSWORD=secret
_JOB_EXECUTION_JSON=$(curl -s "http://${_USER}:${_PASSWORD}@${_HOSTNAME_PORT}/batch/allDocumentUploadJob/start")
...
do
	...
	then
		echo "allDocumentUploadJob starting..."
		sleep 5
	else
		break
	fi
...
then
	echo "Job allDocumentUploadJob unsuccessfully started. Job status is ${_JOB_EXECUTION_STATUS}." 
	exit 1
else
	echo "Job allDocumentUploadJob successfully started."
fi
...
do
	...
	if [ "${_JOB_EXECUTION_STATUS}" = "STARTED" ]
	then
		echo "Job allDocumentUploadJob still running."
		sleep 5
		continue
	elif [ "${_JOB_EXECUTION_STATUS}" = "COMPLETED" ]
	then
		echo "Job allDocumentUploadJob completed."
		exit 0
	else
		echo "Unknown job status (${_JOB_EXECUTION_STATUS}) for job allDocumentUploadJob. Exiting."
		exit 1
	fi
done
