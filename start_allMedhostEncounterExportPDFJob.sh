...
_USER=admin
_PASSWORD=secret
_JOB_EXECUTION_JSON=$(curl -s "http://${_USER}:${_PASSWORD}@${_HOSTNAME_PORT}/batch/allMedhostEncounterExportPDFJob/start")
...
do
	...
	then
		echo "allMedhostEncounterExportPDFJob starting..."
		sleep 5
	else
		break
	fi
...
then
	echo "Job allMedhostEncounterExportPDFJob unsuccessfully started. Job status is ${_JOB_EXECUTION_STATUS}." 
	exit 1
else
	echo "Job allMedhostEncounterExportPDFJob successfully started."
fi
...
do
	...
	if [ "${_JOB_EXECUTION_STATUS}" = "STARTED" ]
	then
		echo "Job allMedhostEncounterExportPDFJob still running."
		sleep 5
		continue
	elif [ "${_JOB_EXECUTION_STATUS}" = "COMPLETED" ]
	then
		echo "Job allMedhostEncounterExportPDFJob completed."
		break	
	else
		echo "Unknown job status (${_JOB_EXECUTION_STATUS}) for job allMedhostEncounterExportPDFJob. Exiting."
		exit 1
	fi
done
